...
#   NGL=999 ./run_llama_cpp.sh ...           # GPU layers (default: 999 = all; use 0 for CPU)
...
#   CPU_ONLY=1 ./run_llama_cpp.sh ...         # force CPU-only (hides GPU, avoids CUDA OOM)
#
# Stop:
...
# Choose GPU or CPU mode based on NGL and CPU_ONLY settings
if [ "${CPU_ONLY}" = "1" ] || [ "${NGL}" = "0" ]; then
    echo "Mode  : CPU-only (CUDA hidden to avoid OOM)"
    docker run -d \
        --name "$NAME" \
        -p "${PORT}:8080" \
        ...
        -ngl 0 -c "$CTX" -np "$NP" $EXTRA_ARGS
else
    echo "Mode  : GPU (NGL=$NGL)"
    docker run -d \
        --name "$NAME" \
        --runtime=nvidia \
        -p "${PORT}:8080" \
        ...
        $MMPROJ_ARGS \
        --host 0.0.0.0 --port 8080 \
        -ngl "$NGL" -c "$CTX" -np "$NP" -fa on --cache-reuse 256 $EXTRA_ARGS
fi
