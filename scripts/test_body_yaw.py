...
class MockReachy:
    def look_at_image(self, x, y, duration, perform_movement=False):
        return np.eye(4) # Επιστρέφει ένα identity matrix αντί για pose
    def set_target(self, head=None, antennas=None, body_yaw=None):
        pass # Απλή παράκαμψη για να μην κρασάρει
...
def _stream():
    while cmd["run"]:
        # Εδώ η κίνηση του ρομπότ αγνοείται γιατί δεν υπάρχει το hardware
        time.sleep(0.01)
threading.Thread(target=_stream, daemon=True).start()
time.sleep(2.0)

# Προσοχή: αφαιρέσαμε το kill_stale_camera_holders που είναι SDK specific
