...
# Στόχευση του επιβεβαιωμένου video index
CAMERA_INDEX = 0
...
def capture_frame(cap, width, height, jpeg_quality):
    ...
    # Warmup frames για τον αισθητήρα
    for _ in range(5):
        cap.read()
    ret, frame = cap.read()
    if not ret:
...
def _open_camera():
    """Παράκαμψη του Reachy SDK και απευθείας άνοιγμα μέσω V4L2 backend."""
    print(f"Attempting to open camera /dev/video{CAMERA_INDEX} via V4L2...")
    cap = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_V4L2)
    
    if not cap.isOpened():
        print("Fallback to standard VideoCapture index...")
